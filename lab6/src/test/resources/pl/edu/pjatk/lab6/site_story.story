...
When user inserts password: ZLE_HASLO in input with name: Password
...
When user inserts password: P@ssw0rd in input with name: Password
When user clicks button with class name: btn-default
Then site visible button Wyloguj in right upper corner after trying to log

Scenario: User is checking if his session is saved on site
Given user is on Home page
When user clicks on link with text: GunShop
Then site contains link with text: Wyloguj

Scenario: User is checking if his session is destroyed after logout
Given user is on Home page
When user clicks on link with text: Wyloguj
